...
const markdownAgentsDir = path.join(repoRoot, 'agents');
...
if (
  test('reference config wires every generated Codex role file', () => {
    const markdownAgents = fs
      .readdirSync(markdownAgentsDir)
      .filter(file => file.endsWith('.md'))
      .sort();

    for (const markdownAgent of markdownAgents) {
      const roleFile = markdownAgent.replace(/\.md$/, '.toml');
...
if (
  test('generated Codex role configs include a generated-file header', () => {
    const roleFiles = fs
      .readdirSync(markdownAgentsDir)
      .filter(file => file.endsWith('.md'))
      .map(file => file.replace(/\.md$/, '.toml'));
    assert.ok(roleFiles.length > 0, 'Expected generated markdown-backed role config files under `.codex/agents`');

    for (const roleFile of roleFiles) {
      const rolePath = path.join(codexAgentsDir, roleFile);
      const roleConfig = fs.readFileSync(rolePath, 'utf8');
      assert.match(roleConfig, /^# Generated by scripts\/codex\/convert-agents-to-toml\.js/m);
      assert.match(roleConfig, /^# Source: agents\/.+\.md/m);
    }
  })
)
  passed++;
